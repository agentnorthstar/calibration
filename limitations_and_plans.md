...
**CCIP capability gap: aggregate signals only, per-message capture on near-term roadmap.**
CCIP lanes today expose `messages_confirmed_1h`, `last_sequence_advance_s`, and `rmn_cursed`. Per-message latency (P90/P99 of send → execute) and per-message attestation matching are **not yet computed**. CCIP `capability_level` in the panel is `aggregate`, while CCTP is `per_message_attested`. This asymmetry is acknowledged as a known limitation as of 2026-05-11 and is targeted for resolution in May 2026. The planned upgrade implements the same pending-queue per-message capture pattern proven on CCTP, adapted to CCIP's ABI structure (`CCIPSendRequested` → `ExecutionStateChanged` matching by `messageId`). A known minor defect (`sequence_gap` field currently always `NULL` in the CCIP collector because the `CCIPSendRequested` event has no indexed parameter from which `topics[1]` could be read) is naturally resolved by this refactor.
...
- **May 2026** — CCIP per-message capture (`CCIPSendRequested` → `ExecutionStateChanged` matching by `messageId`, with pending queue). Brings CCIP `capability_level` from `aggregate` to `per_message_attested`, matching CCTP coverage depth. Resolves the `sequence_gap = NULL` known defect as a natural side-effect.
- **Late May / early June 2026** — CCIP crypto-grounding upgrade: capture of DON threshold-signature `CommitReport` payload (CCIP analogue of Circle ECDSA on CCTP). Per-message verifiability via Merkle inclusion proof against the captured commit root.
