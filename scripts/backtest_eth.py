...
Source: eth_invariants_2020_2024_phi280.csv (BigQuery, Φ=280 blocks, 2020-2024)

Protocol:
  1. Load the invariants
  2. Compute EMA fast/slow on rho_ts and rho_s
...
# ─────────────────────────────────────────────
# ETH PARAMETERS (empirical, confidence: LOW)
# ─────────────────────────────────────────────
...
THRESHOLD_S2     = 1.08         # rhythm_ratio > threshold → S2 (rho_ts)
THRESHOLD_D2     = 1.05         # sigma_ratio  > threshold → D2 (rho_s — ETH ultra-stable)
...
GROUND_TRUTH = [
    {
        ...
        "notes"          : "Gas demand peak / fee congestion. Expected π signal.",
    },
    {
        "name"           : "NFT Mania",
        "onset"          : "2021-03-01",
        ...
        "expected_states": ["S1D2", "S2D2"],
        "notes"          : "CryptoPunks / Bored Apes. Fee congestion. π signal.",
    },
    {
        "name"           : "The Merge",
        "onset"          : "2022-09-14",
        ...
        "notes"          : "PoW→PoS transition. Structural τ stress without π surge. Canonical S2D1 case.",
    },
    {
        ...
        "notes"          : "Withdrawals activation. Possible τ perturbation.",
    },
]
...
print("[2] Computing EMA fast/slow ...")
...
for evt in GROUND_TRUTH:
    ...
    # Detection window: onset → window_end
    mask_window = (df["dt"] >= onset) & (df["dt"] <= win_end)
    window_df   = df[mask_window]

    # Pre-onset window: 24h before → onset (to measure noise)
    ...
    # Detection ratio within the window
    ...
    print(f"    {status}  {evt['name']:25s}  {lat_str}  (ratio {detect_ratio:.0%} within the window)")

# ─────────────────────────────────────────────
# 5. FPR — NOISE OUTSIDE EVENTS
# ─────────────────────────────────────────────

# "Outside ground truth events" mask
...
print(f"""
╔══════════════════════════════════════════════════════╗
║  BACKTEST ETH 2020–2024 — SUMMARY
╠══════════════════════════════════════════════════════╣
║  Invariants analyzed : {len(df):>6,}  (Φ=280, ~1h/inv)
║  Parameters          : S2={THRESHOLD_S2}  D2={THRESHOLD_D2}
║  EMA fast α          : {ALPHA_FAST:.4f}  (~10h)
║  EMA slow α          : {ALPHA_SLOW:.5f} (~30d)
╠══════════════════════════════════════════════════════╣
