...
Source : pol_invariants_2020_2024_phi1800.csv (BigQuery, Φ=1800 blocs, 2020-2024)

Protocole :
  1. Charger les invariants
  2. Calculer EMA fast/slow sur rho_ts, rho_s, size_avg, tx_count_avg
  3. Classifier chaque fenêtre : S1D1 | S1D2 | S2D1 | S2D2
     Logique D2 : ≥2 dims sur 3 (sigma, size, tx) au-dessus de leur seuil
...
# ─────────────────────────────────────────────
# PARAMÈTRES POLYGON (valeurs P90/P95 — à calibrer)
# ─────────────────────────────────────────────
...
THRESHOLD_S2     = 1.04034      # rhythm_ratio > seuil → S2 (P90 initial)
THRESHOLD_SIGMA  = 1.13594      # P95 initial
THRESHOLD_SIZE   = 1.17667      # P95 initial
THRESHOLD_TX     = 1.23474      # P95 initial
...
# ─────────────────────────────────────────────
# GROUND TRUTH — événements connus Polygon
...
GROUND_TRUTH = [
    {
        "name"           : "Network Halt Mars 2021",
        "onset"          : "2021-03-11",
        "window_end"     : "2021-03-12",
        "expected_states": ["S2D1", "S2D2"],
        "notes"          : "Halt réseau ~11h. Stress τ pur — infrastructure.",
    },
    {
        "name"           : "Gas Crisis Mai 2021",
        "onset"          : "2021-05-01",
        "window_end"     : "2021-06-30",
        "expected_states": ["S1D2", "S2D2"],
        "notes"          : "Gas fees x10-100. Congestion demande NFT/DeFi sur Polygon.",
    },
    {
        ...
        "expected_states": ["S2D1", "S2D2"],
        "notes"          : "Bug consensus Heimdall, ralentissement ~11h. Stress τ.",
    },
    {
        ...
        "expected_states": ["S2D1", "S2D2"],
        "notes"          : "157 blocs reorg. Perturbation structurelle rho_ts.",
    },
]
...
# Distribution c_s (pour confirmer continuity_p10 = null sur Polygon)
cs = df["c_s"].dropna()
print(f"\n    Distribution c_s : p10={cs.quantile(.10):.4f}  p50={cs.quantile(.50):.4f}  p90={cs.quantile(.90):.4f}")
if cs.quantile(.10) > 0.99:
    print("    ✅ c_s invariant ~1.0 sur Polygon — continuity_p10 = null confirmé")
else:
    print(f"    ⚠️  c_s p10 < 0.99 ({cs.quantile(.10):.4f}) — variance détectée, à investiguer")

# ─────────────────────────────────────────────
# 2. EMA SÉQUENTIELLES
# ─────────────────────────────────────────────

print("\n[2] Calcul EMA fast/slow ...")
...
# Logique π : D2 si ≥2 dims sur 3 au-dessus de leur seuil
...
print(f"\n    Distributions normales (hors événements) :")
for col in ["rhythm_ratio", "sigma_ratio", "size_ratio", "tx_ratio"]:
...
fig2.suptitle("Invarians Polygon — Distributions signaux", color="white", fontsize=11)
...
print(f"""
╔══════════════════════════════════════════════════════╗
║  BACKTEST POLYGON 2020–2024 — RÉSUMÉ
╠══════════════════════════════════════════════════════╣
║  Invariants analysés : {len(df):>6,}  (Φ=1800, ~1h/inv)
║  Paramètres S2 : {THRESHOLD_S2}  σ={THRESHOLD_SIGMA}  sz={THRESHOLD_SIZE}  tx={THRESHOLD_TX}
