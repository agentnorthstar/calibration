...
Phase B: continuity_p10 verification (c_s distribution)
Goal: FPR < 1.5%, TPR max on known structural events
"""
...
ALPHA_FAST      = 2 / 11
THRESHOLD_SIGMA = 1.13594   # P95 initial — fixed for Phase A
THRESHOLD_SIZE  = 1.17667   # P95 initial — fixed for Phase A
THRESHOLD_TX    = 1.23474   # P95 initial — fixed for Phase A
WARMUP_INV      = 50

# Structural events (τ) — expected S2Dx
GROUND_TRUTH_TAU = [
...
# π fixed (for computation FPR combine during Phase A)
...
for thr in THRESHOLDS_S2:
    ...
    for evt in GROUND_TRUTH_ALL:
        ...
        if evt["name"] == "Network Halt March 2021" and len(det):
...
if cs.quantile(.10) > 0.99:
    ...
    print("    → Relancer with sweep continuity_p10 [0.80 → 0.98]")
...
if best:
    print(f"    recommended threshold_s2: {best['threshold_s2']}")
    print(f"    FPR τ only              : {best['fpr']:.2%}")
    print(f"    TPR τ                   : 3/3 events structural")
else:
    # Fallback : max TPR
    ...
    print(f"    ⚠️  TPR < 3/3 — best trade-off: threshold_s2={best['threshold_s2']}")
