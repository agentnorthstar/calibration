"""
Invarians — Full D2 sweep ETH (sigma + size + tx)
threshold_s2 = 1.12 fixed (validated)
threshold_d2 sigma = 1.10 fixed (validated)
Goal: validate size_demand and tx_demand (currently P95 from the old backtest)
Production logic: D2 if 2 out of 3 dims are above their respective thresholds
"""
...
ALPHA_FAST    = 2 / 11
THRESHOLD_S2  = 1.12
SIGMA_DEMAND  = 1.10   # validated
WARMUP_INV    = 50

# Thresholds to test for size and tx
...
print("[2] Compute EMA (rho_ts, rho_s, size_avg, tx_count_avg) ...")
...
print("\n[3] Normal distributions (excluding events):")
...
fig.suptitle("ETH — Sweep D2 : size_demand × tx_demand  (sigma=1.10 fixed, S2=1.12 fixed)",
