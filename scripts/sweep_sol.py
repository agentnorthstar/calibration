"""
Invarians — τ sweep Solana (rhythm_p90 + continuity_p10)

Solana has two OR-combined structural signals:
  struct_stress = rhythm_ratio > threshold_s2  OR  c_s < continuity_p10

Phase A: sweep threshold_s2 only (continuity_p10 fixed high = deactivated)
Phase B: sweep continuity_p10 only (threshold_s2 fixed to Phase A value)
Phase C: optimal combined verification

Target structural events:
  - Outage Sept 2021 (~17h)  → S2D1
  - Outage Jan  2022 (~4h)   → S2D1
  - Outage May  2022 (~4.5h) → S2D1
...
# Phase A: sweep rhythm_p90, continuity disabled (very high value)
THRESHOLDS_S2      = [1.01, 1.02, 1.03, 1.04, 1.05, 1.08, 1.10, 1.12, 1.15, 1.18, 1.20]
CONTINUITY_FIXED   = 0.50   # P10 very low = effectively disabled for Phase A

# Phase B: sweep continuity_p10, threshold_s2 fixed after Phase A
# (will be replaced at runtime by the Phase A result)
THRESHOLDS_CONT    = [0.80, 0.85, 0.88, 0.90, 0.92, 0.94, 0.95, 0.96, 0.97, 0.98]

# Structural events only for τ
STRUCTURAL_EVENTS = [
...
# ── Loading + gaps + EMA ───────────────────────────────────────────────────
print("[1] Loading + remplissage gaps ...")
...
# ── Distributions the signals en conditions normales ──────────────────────────
print("\n[3] Distributions signals normaux :")
...
print(f"\n[4] Phase A — Sweep threshold_s2 (continuity_p10={CONTINUITY_FIXED} disabled)")
...
for ts2 in THRESHOLDS_S2:
    # struct_stress = rhythm_ratio > ts2 OR c_s < CONTINUITY_FIXED (≈ never)
...
# ── Automatic selection of Phase A value ─────────────────────────────────────
# Best value: all detections + FPR < 3%
best_a = None
...
if best_a:
    ...
    print(f"\n→ Phase A : no automatically-found optimal threshold, using {THRESHOLD_S2_OPT}")

# ── Phase B : sweep continuity_p10 ───────────────────────────────────────────
print(f"\n[5] Phase B — Sweep continuity_p10 (threshold_s2={THRESHOLD_S2_OPT} fixed)")
...
axes[0].axhline(1.5, color="#ff5555", lw=0.8, ls="--", label="FPR target 1.5%")
axes[0].axhline(3.0, color="#ffaa55", lw=0.8, ls="--", label="FPR target 3.0%")
if best_a:
...
axes[1].axhline(1.5, color="#ff5555", lw=0.8, ls="--", label="FPR target 1.5%")
...
print("\nSweep τ termine. Lance sweep_sol_d2.py for calibrer π.")
