...
OUT_JSON = RESULTS / "delta_full_exploration_output.json"
OUT_MD = RESULTS / "delta_full_exploration_report.md"
...
def main():
    ...
    # Per-direction bridge stress for cross-chain
    df["bridge_eth_to_arb"] = (
        (df["bridge_state_eth_arb"] == "BS2") |
        (bridge_ratios[["cctp_eth_arb_latency_p90_s", "cctp_eth_arb_latency_p99_s"]].max(axis=1) >= BRIDGE_LATENCY_RATIO_THRESHOLD).fillna(False)
    )
    df["bridge_arb_to_eth"] = (
        (df["bridge_state_arb_eth"] == "BS2") |
        (bridge_ratios[["cctp_arb_eth_latency_p90_s", "cctp_arb_eth_latency_p99_s"]].max(axis=1) >= BRIDGE_LATENCY_RATIO_THRESHOLD).fillna(False)
    )
    ...
    t0 = time.time()
    F4_count = 0
    # Direction 1: ETH shifts -> bridge_eth_to_arb (predict bridge to ARB)
    # Direction 2: ARB shifts -> bridge_arb_to_eth (predict bridge to ETH)
    for direction, axes, outcome_col in [
        ("ETH_predict_eth_to_arb_bridge", ETH_SHIFTS, "bridge_eth_to_arb"),
        ("ARB_predict_arb_to_eth_bridge", ARB_SHIFTS_EFFECTIVE, "bridge_arb_to_eth"),
    ]:
        for axis in axes:
            threshold = axis_thresholds[axis][PCTL_FIXED]
            instant = (smd[axis] >= threshold).fillna(False).astype(int)
