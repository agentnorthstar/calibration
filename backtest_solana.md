...
backtest_fpr_tau_ci95: "[0.0170, 0.0184]"   # Clopper-Pearson exact — k=2254, n=127354
backtest_n_normal: 127354
backtest_n_false_alarms: 2254
...
> - **TPR_τ = 100% (4/4) — IC95% Clopper-Pearson: [39.76% ; 100.00%]**
> - **FPR_τ = 1.77% (2254/127,354) — IC95% Clopper-Pearson: [1.70% ; 1.84%]**
>
...
>
> The wide TPR interval reflects the small event count (n=4). Reproduction: `python scripts/ci_binomial.py`.
...
|------|---------------|---|---|----------------------|
| **TPR_τ** (outages detected / outages) | 100.00% | 4 | 4 | **[39.76% ; 100.00%]** |
| **FPR_τ** (false alarms / normal windows) | 1.77% | 2,254 | 127,354 | **[1.70% ; 1.84%]** |

**Interpretation notes:**

- The TPR_τ CI is wide because n=4 outages. Even a perfect 4/4 is statistically compatible with a true rate as low as ~40%. Enlarging n is the only way to tighten the interval.
- The FPR_τ CI is very narrow (±0.07%) because n_normal ≈ 127,000 (Solana has ~3.6× more windows than ETH due to shorter Φ).
- π is not yet calibrated (see scope above) — no TPR/FPR for π is published here.

**Reproduction:**

```bash
python scripts/ci_binomial.py --k 4 --n 4
python scripts/ci_binomial.py --k 2254 --n 127354
```
...
![SOL τ ROC](scripts/roc_sol.png)

Generated from the 1D τ sweep phase A (`sol_sweep_results.csv`, 20 threshold points). TPR is computed over all four Solana outages (September 2021, January 2022, May 2022, October 2022).

**AUC = 0.994** — The operating point (τ = 1.12, FPR = 1.77%, TPR = 100%) sits at the best achievable (TPR, FPR) tradeoff on the sweep grid. Any threshold stricter than 1.12 loses the May 2022 outage first (progressive, lower-amplitude event).
