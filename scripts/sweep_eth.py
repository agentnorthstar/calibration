...
Goal: find the optimal S2 threshold for ETH (FPR < 2%, TPR max)
"""
...
# ── Loading + EMA (computed only once) ───────────────────────────────────
...
for thr in THRESHOLDS_S2:
    ...
    # FPR — use the df_normal index on df (slice = copy, not a view)
...
# Zone target FPR < 2%
ax.axhline(y=2.0, color="#4a7c59", linewidth=1, linestyle="--", label="target FPR < 2%")
ax.axhline(y=1.0, color="#4a7c59", linewidth=0.6, linestyle=":", label="target FPR < 1%")

# Annoter each point : TP Merge ?
...
ax.set_title("ETH — FPR vs threshold_s2  (D2=1.05 fixed)", color="white", fontsize=11)
