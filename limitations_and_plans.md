...
CCIP lanes calibration was attempted at the preliminary stage on 2026-05-04 (Entry #037) and explicitly deferred. The continuously-filled observable `last_sequence_advance_s` saturates at the collector cap on 8 of 10 lanes, indicating throughput on the monitored lanes is below the threshold for statistical baseline. CCIP lanes are exposed in the panel as raw observability entries (`state: null`, `calibrated: false`, `status: "UNCALIBRATED"`), with their raw signals visible to consumers. Classification will activate when sustained throughput emerges, expected as institutional RWA cross-chain settlement adoption on CCIP grows. Timeline contingent on observed lane volume.
